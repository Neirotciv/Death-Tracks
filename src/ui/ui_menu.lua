...
function ui_menu:init()
  self.frame_mainMenu = ui.newFrame(0, 0, screenWidth, screenHeight)
    self.frame_mainMenu:setBackgroundColor(0.1, 0.15, 0.19, 1)
  
  self.btn_newRace = ui.newButton(screenWidth/2-100, screenHeight/2-95, 200, 50, "NEW RACE", font)
    self.btn_newRace:setEvent("pressed", newRace)
  self.btn_options = ui.newButton(screenWidth/2-100, self.btn_newRace.y + 70, 200, 50, "OPTIONS", font)
  self.btn_quit = ui.newButton(screenWidth/2-100, self.btn_options.y + 70, 200, 50, "QUIT", font)
    self.btn_quit:setEvent("pressed", quit)
    
  -- Si le joueur ouvre le menu pendant la course, remplacer New Race par Continue
  self.btn_continue = ui.newButton(screenWidth/2-100, screenHeight/2-95, 200, 50, "CONTINUE", font)
  
  self.frame_mainMenu:addElement(screenWidth/2-100, screenHeight/2-95, self.btn_newRace)
  --self.frame_mainMenu:addElement(screenWidth/2-100, screenHeight/2-95, self.btn_continue)
  --self.frame_mainMenu:addElement(screenWidth/2-100, self.btn_newRace.y + 70, self.btn_options)
  self.frame_mainMenu:addElement(screenWidth/2-100, self.btn_options.y + 70, self.btn_quit)
    
  self.frame_optionsMenu = ui.newFrame(0, 0, screenWidth, screenHeight)
    self.frame_optionsMenu:setBackgroundColor(0.1, 0.15, 0.19, 1)
    
  self.frame_raceMenu = ui.newFrame(0, 0, screenWidth, screenHeight)
    self.frame_raceMenu:setBackgroundColor(0.1, 0.15, 0.19, 1)
    
  self.frame_upgradeMenu = ui.newFrame(0, 0, screenWidth, screenHeight)
